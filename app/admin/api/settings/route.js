...
export async function GET() {
  ...
  try {
    const [orgResult, settingsResult, brandingResult] = await Promise.all([
      supabase
        .from('organizations')
...
export async function POST(request) {
  ...
  try {
    ...
    const { org = {}, settings = {}, branding = {} } = body
    ...
    const [orgResult, settingsResult, brandingResult] = await Promise.all([
      ...
      supabase
        .from('organization_settings')
        .upsert(settingsUpsert, { onConflict: 'organization_id' }),
      supabase
        .from('organization_branding')
        .upsert(brandingUpsert, { onConflict: 'organization_id' }),
    ])
