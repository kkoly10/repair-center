...
export async function getPaymentSummaryByQuoteId(quoteId, orgId = null) {
  const supabase = getSupabaseAdmin()

  let query = supabase.from('quote_requests').select('*').eq('quote_id', quoteId)
  if (orgId) query = query.eq('organization_id', orgId)
  const { data: quoteRequest, error: quoteError } = await query.maybeSingle()
