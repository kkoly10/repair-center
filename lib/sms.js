...
async function insertSMSLog({ supabase, eventKey, dedupeKey, quoteRequestId, repairOrderId, recipientPhone, body }) {
...
async function sendSMS({ supabase, eventKey, dedupeKey, quoteRequestId, repairOrderId, to, body }) {
...
export async function sendEstimateSMS({ supabase, quoteRequestId, estimateId, totalAmount }) {
  ...
  const body = `Hi ${context.firstName}, your repair estimate for ${device} is ready! Review it here: ${context.reviewUrl} - Repair Center`
...
export async function sendDepositConfirmSMS({ supabase, quoteRequestId, repairOrderId, paymentIntentId, depositAmount }) {
  ...
  const body = `Hi ${context.firstName}, your $${amount} deposit for ${device} repair has been received. View mail-in instructions: ${context.mailInUrl} - Repair Center`
...
export async function sendRepairStatusSMS({ supabase, quoteRequestId, repairOrderId, historyId, status }) {
  ...
  const body = `Hi ${context.firstName}, your repair status has been updated to: ${statusText}. Track progress: ${context.trackingUrl} - Repair Center`
...
export async function sendShipmentSMS({ supabase, quoteRequestId, repairOrderId, shipmentId, trackingNumber }) {
  ...
  const device = context.deviceDescription || 'your device'
  const body = `Hi ${context.firstName}, your repaired ${device} has shipped! Tracking: ${trackingNumber || 'pending'}. Track: ${context.trackingUrl} - Repair Center`
...
export async function sendFinalBalanceSMS({ supabase, quoteRequestId, repairOrderId, amountDue }) {
  ...
  const body = `Hi ${context.firstName}, your repair is complete! Final balance of $${amount} is due. Pay here: ${payUrl} - Repair Center`
