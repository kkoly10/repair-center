...
async function insertNotificationLog({ supabase, dedupeKey, organizationId, quoteRequestId, repairOrderId, paymentId, recipientEmail, subject, payload }) {
  try {
    if (dedupeKey) {
    ...
    console.error('[final-balance-paid] log insert failed:', error)
    return null
  }
}
...
async function updateNotificationLog(supabase, notificationId, updates) {
  ...
  try {
    ...
    console.error('[final-balance-paid] log update failed:', error)
  }
}

export async function sendFinalBalancePaidNotification({
  supabase,
  quoteRequestId,
  repairOrderId,
  paymentId,
  ...
  const subject = orderNumber
    ? `Final balance received – Order #${orderNumber}`
    : `Final balance received – Quote #${context.quoteRequest.quote_id}`

  const dedupeKey = `final-balance-paid:${repairOrderId}:${paymentId || amount}`
  ...
  if (log?.duplicate) return { ok: true, skipped: true, reason: 'duplicate' }
    ...
    <body style="margin:0;padding:24px;background:#f3f4f6;font-family:Arial,sans-serif;color:#111827;">
      <div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;">
        <div style="padding:24px 28px;background:#111827;color:#fff;">
          <div style="font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#cbd5e1;">${esc(context.orgName || 'Repair Center')}</div>
          <h1 style="margin:8px 0 0;font-size:28px;line-height:1.15;">Final balance received</h1>
          <p style="margin:12px 0 0;font-size:15px;line-height:1.6;color:#e5e7eb;">Hi ${esc(context.firstName)}, we received your remaining repair balance.</p>
        </div>
        <div style="padding:28px;">
          <p style="margin:0 0 18px;font-size:15px;line-height:1.6;color:#374151;">Amount received: <strong>${esc(fmt(amount))}</strong></p>
          ...
          <p style="margin:0 0 18px;font-size:15px;line-height:1.6;color:#374151;">Your order is now financially cleared and can move into final shipping.</p>
  ...
  try {
    ...
    console.error('[final-balance-paid] send failed:', error)
