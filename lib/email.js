...
function receiptHtml({ org, quote, customer, order, estimate, line_items, payments }) {
  ...
  const paymentsHtml = payments?.length
    ? `<div style="margin-top:24px;">
        <p style="margin:0 0 12px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;color:#999;">Payments received</p>
        ${payments.map((p) => `
          <div style="display:flex;justify-content:space-between;font-size:14px;color:#555;padding:6px 0;border-bottom:1px solid #f0f0f0;">
            <span>${p.kind?.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()) || 'Payment'}</span>
            <span style="font-weight:600;color:#111;">${fmt(p.amount)}</span>
          </div>
        `).join('')}
      </div>`
    : ''
