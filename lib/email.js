...
export async function sendAppointmentConfirmationEmail({ to, orgName, firstName, preferredAt, device, repairDescription }) {
  ...
  await resend.emails.send({
    ...
    html: emailWrapper(`
      ...
      <p style="margin:0 0 24px;font-size:15px;color:#555;">Hi ${firstName}, we've received your appointment request and will confirm it shortly.</p>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8f8f8;border-radius:6px;padding:16px 20px;margin-bottom:24px;">
        <tr><td>
          ...
          <p style="margin:0 0 12px;font-size:15px;color:#111;font-weight:600;">${dateStr}</p>
          ...
          <p style="margin:0 0 12px;font-size:15px;color:#111;">${device}</p>` : ''}
          ...
          <p style="margin:0;font-size:14px;color:#555;">${repairDescription}</p>` : ''}
        </td></tr>
      </table>
      ...
      <p style="margin:24px 0 0;font-size:13px;color:#999;">— ${orgName}</p>
    `, `You received this because you submitted an appointment request to ${orgName}.`),
  })
}
