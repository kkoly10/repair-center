...
export async function sendEstimateReadyEmail({ to, customerName, quoteId, deviceDescription, totalAmount, locale = 'en' }) {
  if (!resend) {
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: `${t('email.quoteSubject')} – #${quoteId}`,
...
export async function sendShippingNotificationEmail({
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: `${t('email.statusSubject', { status: t('status.shipped') })}${orderNumber ? ` – #${orderNumber}` : ''}`,
...
export async function sendNewQuoteAlertEmail({ to, orgName, quoteId, customerName, device, repairType, locale = 'en' }) {
  if (!resend) {
  ...
  await resend.emails.send({
    ...
    subject: `${t('email.newQuoteAlertSubject')} – ${device || 'Device'}`,
    html: emailWrapper(`
...
export async function sendTrialExpiryWarningEmail({ to, orgName, daysLeft, billingUrl, locale = 'en' }) {
  if (!resend) {
  ...
  const subject = expired
    ? `${t('email.trialEndedSubject')} – ${orgName}`
    : `${t('email.trialWarningSubject', { days: daysLeft })} – ${orgName}`
...
export async function sendReceiptEmail({ to, invoice, locale = 'en' }) {
  if (!resend) {
  ...
  const subject = order?.order_number
    ? `${t('email.receiptSubject')} – #${order.order_number} | ${org.name}`
    : `${t('email.receiptSubject')} – ${quote.quote_id} | ${org.name}`
...
const LOCALE_TO_BCP_EMAIL = { en: 'en-US', fr: 'fr-FR', es: 'es-ES', pt: 'pt-PT' }

export async function sendAppointmentConfirmationEmail({ to, orgName, firstName, preferredAt, device, repairDescription, locale = 'en' }) {
  if (!resend) {
  ...
  const t = getTranslator(locale)
  const bcp = LOCALE_TO_BCP_EMAIL[locale] || 'en-US'
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to,
    subject: `${t('email.appointmentSubject')} – ${orgName}`,
    html: emailWrapper(`
