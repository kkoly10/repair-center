...
async function insertNotificationLog({ supabase, eventKey, dedupeKey, organizationId, quoteRequestId, repairOrderId, recipientEmail, subject, payload }) {
...
async function sendFollowUpEmail({ supabase, eventKey, dedupeKey, organizationId, quoteRequestId, repairOrderId, to, subject, html, payload }) {
...
// FTC Consumer Reviews and Testimonials Rule compliance:
// - Review request emails are sent to ALL customers whose repair shipped, not just
//   customers expected to leave a positive review. No sentiment filtering.
// - The email does NOT offer incentives (gift cards, discounts, free repair, etc.)
//   in exchange for a review. Such incentives would have to be both disclosed and
//   not conditioned on review sentiment under the FTC rule.
// - All 5 stars use the same URL pattern (one click each); we do not make
//   high-rating paths easier than low-rating paths.
// - The destination page (/review/[quoteId]) accepts any rating 1–5.
  ...
  const html = wrapEmail({
    ...
    body: `
      ...
      <p style="margin:24px 0 0;font-size:13px;color:#6b7280;">
        Thank you for choosing ${esc(brand)}. Your feedback helps us improve!
      </p>
    `,
  })
...
export async function sendWarrantyReminderEmail({ supabase, quoteRequestId, repairOrderId, warrantyDays }) {
  ...
  const createdAt = context.repairOrder?.repair_completed_at || context.quoteRequest.created_at
