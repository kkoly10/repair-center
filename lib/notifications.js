...
import { generateToken } from './hmacToken'
...
function wrapEmail({ title, intro, body, orgName }) {
  const brand = orgName || 'Repair Center'
  return `<!DOCTYPE html>
  <html lang="en">
    <body style="margin:0;padding:24px;background:#f3f4f6;font-family:Arial,sans-serif;color:#111827;">
      <div style="max-width:640px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;">
        <div style="padding:24px 28px;background:#111827;color:#fff;">
          <div style="font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#cbd5e1;">${esc(brand)}</div>
...
async function insertNotificationLog({ supabase, eventKey, dedupeKey, organizationId, quoteRequestId, repairOrderId, paymentId, recipientEmail, subject, payload }) {
  if (!supabase) return null

  try {
    if (dedupeKey) {
      // Scope by organization + order when available to avoid cross-tenant false
      // positives. Critical for 'repair-status:null' (walk-in intake key), which
      // is a constant shared across all orgs and orders.
      let dedupeQuery = supabase
        .from('notifications')
        .select('id, status')
        .eq('dedupe_key', dedupeKey)
      if (organizationId) dedupeQuery = dedupeQuery.eq('organization_id', organizationId)
      if (repairOrderId) dedupeQuery = dedupeQuery.eq('repair_order_id', repairOrderId)
...
async function sendEmail({ supabase, eventKey, dedupeKey, organizationId, quoteRequestId, repairOrderId, paymentId, to, subject, html, payload }) {
...
function buildSecureUrl(base, quoteId) {
  const tok = generateToken(quoteId)
  return tok ? `${base}?tok=${tok}` : base
}
...
export async function getQuoteNotificationContext(supabase, quoteRequestId) {
  ...
  const [customerResult, orderResult, orgResult] = await Promise.all([
    ...
    supabase
      .from('repair_orders')
      .select('id, order_number, current_status, created_at, repair_completed_at')
      .eq('quote_request_id', quoteRequest.id)
      .maybeSingle(),
    quoteRequest.organization_id
      ? supabase
          .from('organizations')
          .select('public_name, name')
          .eq('id', quoteRequest.organization_id)
          .maybeSingle()
      : Promise.resolve({ data: null, error: null }),
  ])
  ...
  const orgName = org?.public_name || org?.name || null
  ...
  return {
    ...
    reviewUrl: buildSecureUrl(`${BASE_URL}/estimate-review/${quoteRequest.quote_id}`, quoteRequest.quote_id),
    trackingUrl: buildSecureUrl(`${BASE_URL}/track/${quoteRequest.quote_id}`, quoteRequest.quote_id),
    mailInUrl: buildSecureUrl(`${BASE_URL}/mail-in/${quoteRequest.quote_id}`, quoteRequest.quote_id),
  }
}
...
export async function sendEstimateSentNotification({ supabase, quoteRequestId, estimateId, estimateKind, totalAmount }) {
  ...
  return sendEmail({
    supabase,
    eventKey: estimateKind === 'preliminary' ? 'estimate_sent' : estimateKind === 'final' ? 'estimate_final_sent' : 'estimate_revised_sent',
